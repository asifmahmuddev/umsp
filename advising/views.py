...
from .models import AssignedCourse, Course, TakeCourse, Create_Semester, Department
...
def Advising(request):
    ...
    if request.method == 'POST':
        ...
        if dataTakeCourse:
            ...
            messages.error(request, 'Sorry, At first finish prerequisite courses.')
            return JsonResponse({"status": 500})

        elif (total_credit > 15):
            messages.error(request, 'Sorry, You can not take above 15 credit.')
            ...
            messages.error(request, 'Sorry, sit fill up.')
            return JsonResponse({"status": 500})
